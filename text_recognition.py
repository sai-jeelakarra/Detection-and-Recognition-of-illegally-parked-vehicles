# USAGE
# python3 text_recognition.py --east frozen_east_text_detection.pb --image vehicles/multiple1.1_plate.jpg
...
def recognize(imagename):
	...
	for ((startX, startY, endX, endY), text) in results:
		...
		for i in range(1, len(doc)):
			if doc[i][2] == text:
				doc[i][3] = str(int(doc[i][3]) + 10);
				doc[i][1] = dt_string
				flag = 0
				break

		if (flag == 1):
			newrow = [dt_string, dt_string, text, "0"]
			doc.append(newrow)
...
recognize("vehicles/multiple1.2_plate.jpg")
